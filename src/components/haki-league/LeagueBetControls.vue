...
<template>
  <form
    :class="['bg-gray-100 dark:bg-surface-elevated']"
    class="sticky bottom-0 shadow-md w-full py-2.5 space-y-4 border-t border-gray-200 dark:border-transparent"
    @submit.prevent="bet"
  >
    ...
    <div class="flex space-x-2 px-4">
      <div class="flex grow">
        ...
        <div
          ...
          class="bg-brand-selected/80 flex cursor-pointer rounded-md hover:bg-brand-bright/70 text-white uppercase tracking-wide font-semibold text-sm w-full justify-center items-center rounded-0 shadow-sm px-2 py-2.5"
          @click="openLogin"
        >
          Login To Place Bet
        </div>
        <button
          v-else
          type="submit"
          class="bg-brand-selected/80 rounded-md hover:bg-brand-bright/70 text-white uppercase tracking-wide font-extrabold text-sm w-full justify-center items-center rounded-0 shadow-sm px-2 py-2.5"
        >
          <TheButtonSpin v-if="pending" />
